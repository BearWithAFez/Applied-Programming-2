...
namespace MazeGame
{
    ...
    { 
        ...
        public abstract bool IsIn(Point3D other);

        public bool IsIn(Shape other)
        {
            // Transforms
            var trans = (other.Model.Transform as TranslateTransform3D);
            var x = (trans == null) ? 0 : trans.OffsetX;
            var y = (trans == null) ? 0 : trans.OffsetY;
            var z = (trans == null) ? 0 : trans.OffsetZ;

            // Check all corners
            foreach (var corner in other.corners) if (IsIn(Point3D.Add(corner, new Vector3D(x, y, z)))) return true;
            return false;
        }
        #endregion Methods
    }
